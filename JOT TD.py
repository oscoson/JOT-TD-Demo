...
sideBar.add_btn(
    ...
    Tower(1055, 220, 110, 110, 'FireLauncher',FireExplosion, fireLauncher, 150, 200, 10, 0,
          0, 80, False))

sideBar.add_btn(
    1055, 70, boulderLauncher, "test", 300,
    Tower(1055, 70, 110, 110, 'BoulderLauncher',BoulderExplosion, boulderLauncher, 100, 300, 30,
          0, 0, 130, False))

sideBar.add_btn(
    1055, 370, rockLauncher, "test", 100,
    Tower(1055, 370, 110, 110, 'RockLauncher',RockExplosion, rockLauncher, 200, 100, 10, 0,
          0, 40, False))
...
class Level:  # Currently represents our first level. Can be used as the base for future levels
    ...
    def newWave(self):
        ...
        for i in range(self.waveCount):
            self.wave.append(
                Enemy(30, 30, 10+self.waveCount//5, 5, 2, 10, meDown, meLeft, meRight, meUp))
        for i in range(self.waveCount // 2):
            self.wave.append(
                Enemy(30, 30, 20+self.waveCount//4, 15, 3, 20, kekeDown, kekeLeft, kekeRight,
                      kekeUp))
        for i in range(self.waveCount // 3):
            self.wave.append(
                Enemy(30, 30, 30+self.waveCount//3, 10, 4, 20, babaDown, babaLeft, babaRight,
                      babaUp))
        for i in range(self.waveCount // 4):
            self.wave.append(
                Enemy(30, 30, 1+self.waveCount//2, 5, 6, 5, anniDown, anniLeft, anniRight,
                      anniUp))
        for i in range(self.waveCount // 5):
            self.wave.append(
                Enemy(30, 30, 100+self.waveCount, 25, 1, 50, roboDown, roboLeft, roboRight,
                      roboUp))
...
def CircleCollide(x1, y1, r1, x2, y2, r2):
    x1, y1, x2, y2 = (x1+r1/2, y1+r1/2, x2+r2/2, y2+r2/2)
    if hypot(x1 + r1 - x2 - r2, y1 + r1 - y2 - r2) < r1 + r2:
        return True
    else:
        return False

cash = FONT.render('Cash: ' + str(round(self.money)), True, (0, 0, 0))
health = FONT.render('Health: ' + str(self.lives), True, red)
for event in pygame.event.get():
    if event.type == pygame.QUIT:
        exit = True
    for tower in towers:
        if currentUser.cash >= tower.cost:
            tower.placeTower(event, road)
            if tower not in currentUser.towers:
                if tower.placed:
                    currentUser.cash -= tower.cost
                    currentUser.towers.append(tower)

#x, y, w, l, name, img, radius, cost, damage=1, dmgBoost=0, speedBoost=0, attackDelay=1, minimumWave=0, placed=False, specialTraits = []

FireLauncher = Tower(x,y,110,110, 'FireLauncher', os.path.join("Tower Sprites",'FireLauncher 1.png'), 550, 200, 10, 0, 0, 0.1, 0, False)

BoulderLauncher = Tower(x,y,110,110, 'BoulderLauncher', os.path.join("Tower Sprites", 'Bounder Launcher 1.png'), 600, 300, 25, 0, 0, 0.5, 0, False)

RockLauncher = Tower(x,y,110,110, 'RockLauncher', os.path.join("Tower Sprites", 'rock launcher 1.png'), 700, 100, 15, 0, 0, 0.25, 0, False)

SpikedLaunched = Tower(x,y,110,100, 'SpikedLauncher', os.path.join("Tower Sprites", "Spiked Launcher 1.png", 500, 150, 10, 0, 0, 0, 0, False)

Towers = [FireLauncher, BoulderLauncher, RockLauncher, SpikedLauncher]


anni = Enemy(30, 30, 10, 5, 6, 5, anniDown, anniLeft, anniRight, anniUp)
baba = Enemy(30, 30, 20, 10, 4, 20, babaDown, babaLeft, babaRight, babaUp)
keke = Enemy(30, 30, 30, 15, 3, 20, kekeDown, kekeLeft, kekeRight, kekeUp)
me = Enemy(30, 30, 10, 5, 2, 10, meDown, meLeft, meRight, meUp)
robo = Enemy(30, 30, 200, 25, 1, 50, roboDown, roboLeft, roboRight, roboUp)
"""
